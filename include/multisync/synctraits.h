...
concept SyncTraitsConcept = requires(
    ...
    std::vector<std::complex<float>>* X     // Vector to store CFR for GetCfr function
) {
    ...
    { SyncTraitsSpecification::GetCfr(synchronizer, X) } -> std::same_as<void>;
};
 ...
 * Define the functions (Create, Reset, Execute, Destroy, GetCfr) for the OFDM frame synchronizer
 * 
 * @tparam  Synchronizer type
 */
template<>
struct SyncTraits<ofdmframesync> {
     ...
     */
    static void GetCfr(SynchronizerType fs, std::vector<std::complex<float>>* X) 
    {
