 ...
 *
 * @brief Reads a pre-generated DVB-S2 ULA dataset (binary file produced by
 * dvbs2_ula_dataset_gen.m) and feeds the per-element baseband samples into the
...
// Number of ULA channels — must match N_rx used in dvbs2_ula_dataset_gen.m
#define NUM_CHANNELS 4

// Binary dataset file written by dvbs2_ula_dataset_gen.m.
// Regenerate whenever N_rx, DOA, roll-off or SNR settings change.
#define DATASET_FILE "simulations/dvbs2/records/dvbs2_sof_N4_DOA30deg_SNR15dB.bin"
...
#define M_FILE "simulations/dvbs2/dvbs2_sim.m"
...
// Reads binary dataset written by dvbs2_ula_dataset_gen.m.
...
int main(int argc, char*argv[])
{
    ...
    // Load pre-generated DVB-S2 ULA dataset from MATLAB
    ...
    // DVB-S2: compose the synchronizer configuration from the preset helpers
    // in sc_standards.h; the preamble is the 26-symbol Start-of-Frame field
    // of the PLHEADER, the PLSC and the first data slots are the payload
    constexpr unsigned int k               = 2;        // samples per symbol
    constexpr unsigned int m               = 7;        // pulse filter delay [symbols]
    constexpr float        beta            = 0.35f;    // RRC roll-off (must match dataset)
    constexpr float        threshold       = 0.5f;     // detection threshold
    constexpr float        dphi_max        = 0.01f;    // carrier offset search range [rad/sample]
    constexpr unsigned int payload_sym_len = 64 + 180; // PLSC (64) + 2 PL slots (2 x 90)
    static liquid_float_complex preamble[DVBS2_SOF_LEN]; // SOF symbols (pi/2-BPSK)
    scframesync_init_dvbs2_sof(preamble);

    SyncWorker<NUM_CHANNELS, scframesync_iface> sync(
        {{preamble, DVBS2_SOF_LEN, LIQUID_FIRFILT_RRC, k, m, beta, threshold, dphi_max, payload_sym_len}},
    ...
    grouping_worker.AddMultiChSymsQueue(std::ref(tx_syms_queue));
